...
import { API_URL, XRPL_API_URL } from "@/constants/constants";
...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const fetchAmendmentsData = async () => {
      try {
        const response = await axios.get(`${XRPL_API_URL}/amendments`);
        if (response.statusText === "OK") {
          const data = response.data;
          setAmendmentData(data);
        }
      } catch (err) {
        setAmendmentError("Failed to fetch amendments data");
    ...
    <main className="px-3 py-4">
      <div className="max-w-[2400px] mx-auto grid gap-4 grid-cols-8 lg:grid-cols-12">
        ...
        <div className="col-span-8 lg:col-span-6 grid grid-cols-2 gap-4">
          ...
          <div className="space-y-4 ">
            <Panel title="Amendments" height="h-[445px]">
              {isLoading ? (
                <div className="flex items-center justify-center h-full text-gray-50">
                  Loading...
                </div>
                ...
                <List list={renderList(amendmentData, Amendment)} />
              )}
            </Panel>
